...
namespace AR.Math.Parsing
{
	public static class Parse
	{
		...
		public static Parser<TInput, T> AtLeastOnce<TInput, T, TSeparator>(
			this Parser<TInput, T> parser, Parser<TInput, TSeparator> separatorParser, 
            Func<T, TSeparator, T, T> aggregator)
		{
			if (parser == null)
				throw new ArgumentNullException(nameof(parser));
			if (separatorParser == null)
			...
			return input =>
			{
				...
				while (result.Success)
				{
					...
					IParsingResult<TInput, TSeparator> dividerResult = separatorParser(input);
					if (!dividerResult.Success)
						return new SuccessParsingResult<TInput, T>(value, input, dividerResult.Error);
					result = parser(dividerResult.Input);
				    if (result.Success)
				        value = aggregator(value, dividerResult.Value, result.Value);
				}
				return new SuccessParsingResult<TInput, T>(value, input, result.Error);
			};
		}
		...
		public static readonly Parser<char, BigInteger> BigInt = 
			(from sign in Char('-').Or(Char('+')).Optional('+')
			from digits in Char(c => c >= '0' && c <= '9', "Digit expected").AtLeastOnce().AsText()
			select BigInteger.Parse(sign + digits)).ErrorMessage("Integer expected").Catch("Integer overflow");
